function phoneAgent() {
    ...
    PhoneAgent.prototype.run = function(command) {
        ...
        while (this.step_count < this.agent_config.max_steps) {
            if (this.stop_step) {
                return "User canceledl current task";
            }
    ...
    PhoneAgent.prototype.stopStep = function() {
        this.stop_step = true;
    }
    ...
    PhoneAgent.prototype.executeStep = function(command, is_first) {
        ...
        if (this.stop_step) {
            return new StepResult(false, true, null, "", "User canceledl current task");
        }
...
==============================
            `)
            this.toastFloaty.show(action.action || action.message || "");
        }
        ...
        let result;
        try {
            result = this.actionHandler.execute(action, screenshot.getWidth(), screenshot.getHeight());
            sleep(1000);
        } catch (e) {
